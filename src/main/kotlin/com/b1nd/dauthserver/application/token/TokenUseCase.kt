...
class TokenUseCase(
    ...
    suspend fun issueToken(request: TokenRequest): ResponseData<TokenResponse> {
        ...
        val accessToken = tokenProvider.generateAccessToken(user.dodamId, application.clientId, user.role.accessLevel)
        val refreshToken = tokenProvider.generateRefreshToken(user.dodamId, application.clientId, user.role.accessLevel)
