...
class AuthUseCase(
    ...
    suspend fun appLogin(request: AppLoginRequest): Response {
        val application: ApplicationEntity = applicationService.getByClientIdAndSecret(request.clientId, request.clientSecret)
        val loginInfo = dodamClient.dodamMy(request.access)
        val newUser = request.toEntity(loginInfo.id, loginInfo.role, listOf(ScopeType.OPENID))
        val user = saveOrUpdateUser(loginInfo.id, request.clientId, request.refresh, listOf(ScopeType.OPENID), newUser)
        return ResponseData.ok("앱 자체 로그인 성공", LoginResponse.of(createCode(user.id!!), application.redirectUrl))
    }
    ...
    suspend fun qrLogin(request: QrLoginRequest): Response {
        ...
        redisService.update(RedisKeyType.QR_LOGIN_CHECKED, request.code, user.id!!.toString())
